    ...
    DB::table('users')
        ...
        ->chunk(100, function ($users) {
            ...
            foreach ($users as $user) {
                ...
                $count = 0; // переменная для счета кол-ва  рейтингов на продукты с ценником больше 3000
                ...
                if ($user->cityUser == 'Volgograd' || $user->cityUser == 'Samara') {
                    foreach ($ratings as $rating) {
                        $currentRating = Rating::find($rating->id); // ищем конкретный рейтинг
                        $currentFeedback = $currentRating->feedback; // находим отзыв у рейтинга (один рейтинг - один отзыв)

                        // проверяем на кол-во отметок "отзыв полезен" - если больше 10, то добавляем в массив
                        if ($currentFeedback->countMarkFeedback > 10) {
                            $array[] = $currentFeedback->id;
                        }
                    }
                } // второй запрос - Получить все отзывы, оставленные пользователями которые оставили более 10 отзывов на товары стоимостью более 3 т.р., при среднем рейтинге всех отзывов пользователя на такие товары более 4.
                else{
                    foreach ($ratings as $rating) {
                        // перебираем продукты
                        foreach ($products as $product) {
                            // просчитываем сколько пользователь оставлял рейтингов на продукты с ценником больше 3000 (на 1 рейтинг есть 1 отзыв)
                            if ($rating->product_id == $product->id) {
                                $count++;
                            }
                        }

                        $currentRating = Rating::find($rating->id);
                        $currentFeedback = $currentRating->feedback; // находим отзыв у рейтинга (один рейтинг - один отзыв)
